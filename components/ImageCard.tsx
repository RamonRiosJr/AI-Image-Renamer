...
const ImageCard: React.FC<ImageCardProps> = ({ imageFile, onNameChange, onRemove }) => {
  ...
  return (
    <div className="relative bg-slate-800 rounded-lg overflow-hidden shadow-lg transition-all duration-300 hover:shadow-cyan-500/20 hover:scale-105 group">
      <button
        onClick={onRemove}
        className="absolute top-2 right-2 z-10 p-1 bg-slate-900/50 rounded-full text-slate-300 hover:text-white hover:bg-red-500/80 transition-all opacity-0 group-hover:opacity-100"
      >
      ...
      <div className="p-4">
        <div className="mb-3">
          ...
          <p className="text-sm text-slate-300 truncate" title={originalName}>
            {originalName}
          </p>
        </div>
        <div>
          <label
            htmlFor={`newName-${imageFile.id}`}

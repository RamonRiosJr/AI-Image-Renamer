  ...
  const handleProcessImages = useCallback(async () => {
    ...
    const processingPromises = images.map(imageFile => {
      if (imageFile.status === 'pending') {
        setImages(prev => prev.map(img => img.id === imageFile.id ? { ...img, status: 'processing' } : img));
        return generateImageName(imageFile.file)
          .then(newName => {
            setImages(prev => prev.map(img => img.id === imageFile.id ? { ...img, newName, status: 'processed' } : img));
          })
          .catch(error => {
            setImages(prev => prev.map(img => img.id === imageFile.id ? { ...img, status: 'error', errorMessage: error.message } : img));
          });
      }
        ...
        <>
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center sticky top-[73px] z-10 bg-slate-900/80 backdrop-blur-sm py-4 rounded-lg">
                ...
                >
                    ...
                    {isProcessing ? 'Processing...' : `Generate Names (${images.filter(i=>i.status==='pending').length})`}
                </button>
                <button
